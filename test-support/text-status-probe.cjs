...
(async () => {
  ...
  const result = await win.webContents.executeJavaScript(`(async () => {
    ...
    assert(status.textContent === '已保存', 'initial TXT status must use one of the two fixed labels');
    const initialEditorTop = editor.getBoundingClientRect().top;
    let saves = 0;
    ...
    assert(settled === '已保存', 'settled successful save must use the fixed saved label');
    assert(finalContent === 'typing-7' && caret === finalContent.length, 'saving disturbed content or caret');
    assert(typing.every(value => value === '编辑中'), 'typing introduced another status label');
    assert(blank.length === 1 && blank[0] === '编辑中' && /空白草稿/.test(status.title), 'empty draft must keep the editing label with details only on hover');
    ...
    assert(status.textContent === '编辑中' && status.title.includes('fixture offline'), 'save failure must stay editing with immediate hover details');
    await wait(1300);
    assert(status.textContent === '编辑中' && status.title.includes('fixture offline'), 'a stale success timer overwrote the error');

    window.eagleMV.saveText = async () => ({ conflict: true });
    input('conflict');
    await saveTextPreview(false, { session, automatic: true });
    const warning = status.title;
    input('conflict edited');
    assert(session.blocked === 'conflict' && status.textContent === '编辑中' && status.title === warning, 'typing hid a conflict warning');
    ...
    await saving;
    assert(session.dirty && status.textContent === blankDuringSave && blankDuringSave === '编辑中' && /空白草稿/.test(status.title), 'snapshot ACK flashed over a later empty draft');
    assert(changes.every(value => ['编辑中', '已保存'].includes(value)), 'status escaped the two-label contract');
    assert(editor.getBoundingClientRect().top === initialEditorTop, 'status length changes moved the editor');
    observer.disconnect();
