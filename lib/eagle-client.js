...
class EagleClient {
  ...
  // API has no color filter, so this runs purely client-side.
  static matchesColor(item, colorValue, threshold = 96) {
    const target = EagleClient.parseHexColor(colorValue);
  ...
  static matchesSearchConstraints(item, query, smartFolderIds = null) {
    ...
    if (query.folderId && !(item.folders || []).includes(query.folderId)) return false;
    if (query.unfiled && (item.folders || []).length) return false;
    if (query.isUntagged && (item.tags || []).length) return false;
    if (query.smartFolderId && (!smartFolderIds || !smartFolderIds.has(item.id))) return false;
    if (query.tags?.length && !query.tags.every(tag => (item.tags || []).includes(tag))) return false;
    if (query.ext && String(item.ext || '').toLowerCase() !== String(query.ext).toLowerCase()) return false;
    if (Number.isInteger(query.rating) && Number(item.star || 0) !== query.rating) return false;
    if (query.annotation?.trim() && !String(item.annotation || '').toLocaleLowerCase('zh-CN').includes(query.annotation.trim().toLocaleLowerCase('zh-CN'))) return false;
    if (query.url?.trim() && !String(item.url || '').toLocaleLowerCase('zh-CN').includes(query.url.trim().toLocaleLowerCase('zh-CN'))) return false;
    if (query.shape) {
      ...
      if (shape !== query.shape) return false;
    }
    if (query.color && !EagleClient.matchesColor(item, query.color)) return false;
    return true;
  }
  ...
  async queryTextItems(query) {
    ...
    const constrained = Boolean(query.folderId || query.smartFolderId || query.unfiled || query.isUntagged ||
      query.tags?.length || query.ext || Number.isInteger(query.rating) || query.annotation?.trim() || query.url?.trim() || query.shape || query.color);
    if (!constrained) {
  ...
  static itemQueryBody(query) {
    ...
    if (query.annotation?.trim()) body.annotation = query.annotation.trim();
    if (query.url?.trim()) body.url = query.url.trim();
    if (query.shape) body.shape = query.shape;
    if (query.isUntagged) body.isUntagged = true;
    return body;
  }
  ...
  async queryItems(query = {}) {
    ...
    if (query.color) return this.queryColorItems(query);
    const body = {
  ...
  // The Eagle API cannot filter by color, so color queries scan the server
  // pages and filter client-side, mirroring queryTextItems' incremental
  // "at least N" semantics for large folders.
  async queryColorItems(query) {
    const offset = Math.max(0, Number(query.offset) || 0);
    const limit = Math.min(500, Math.max(1, Number(query.limit) || 160));
    const matches = [];
    let scanOffset = 0;
    ...
    const needed = offset + limit;
    while (matches.length < needed) {
      const body = { ...EagleClient.itemQueryBody(query), offset: scanOffset, limit: 1000 };
      const page = await this.request('/api/v2/item/get', { method: 'POST', body });
      const data = Array.isArray(page?.data) ? page.data : [];
      matches.push(...data.filter(item => EagleClient.matchesSearchConstraints(item, query)));
      scanOffset += data.length;
      if (!data.length || scanOffset >= Number(page?.total || scanOffset)) {
        exhausted = true;
        break;
