...
class EagleClient {
  ...
  static matchesColor(item, colorValue, threshold = 96) {
    if (EagleClient._parsedColorCache?.key !== colorValue) {
      EagleClient._parsedColorCache = { key: colorValue, rgb: EagleClient.parseHexColor(colorValue) };
    }
    const target = EagleClient._parsedColorCache.rgb;
