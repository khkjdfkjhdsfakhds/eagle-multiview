...
// A valid 5 MB TXT may expand sixfold as JSON escapes. Only text writes get
// this transport allowance; the service still validates decoded content size.
const TEXT_RPC_BODY_LIMIT = 32 * 1024 * 1024;
...
function createWebServer({ srcDir, invoke, resolveMediaPath, ensureLibraryPath = async () => {}, accessKey, requireKey = true, uploadImport, uploadDir, onClientGone, logError, loginFailureDelayMs = LOGIN_FAILURE_DELAY_MS }) {
  ...
  async function handleRPC(req, res) {
    ...
    let bodySize;
    try {
      const body = await readBody(req, TEXT_RPC_BODY_LIMIT);
    ...
    if (bodySize > RPC_BODY_LIMIT && !['text:save', 'text-draft:put'].includes(method)) {
      return sendJSON(res, 413, { ok: false, message: '请求超过 8 MB 限制' });
    }
  ...
  async function prepareExport(rawIds, libraryPath) {
    const ids = [...new Set((Array.isArray(rawIds) ? rawIds : []).map(id => String(id).trim()).filter(Boolean))];
    if (!ids.length) throw Object.assign(new Error('缺少素材 ID'), { status: 400 });
    if (ids.length > EXPORT_ID_LIMIT) throw Object.assign(new Error('一次最多下载 500 个文件，请减少选择后分批下载'), { status: 400 });
    const entries = [];
    const missingIds = [];
    let total = 0;
    for (const id of ids) {
      ...
      const stat = filePath ? await fsp.stat(filePath).catch(() => null) : null;
      if (!stat?.isFile()) { missingIds.push(id); continue; }
      entries.push({ filePath, size: stat.size, mtime: stat.mtime, mtimeMs: stat.mtimeMs, name: path.basename(filePath) });
      total += stat.size;
    }
    await ensureLibraryPath(libraryPath);
    if (missingIds.length) throw Object.assign(new Error(`${missingIds.length} 个原文件缺失，本次未开始下载；请移除缺失项后重试`), { status: 404, missingIds });
    if (total > EXPORT_TOTAL_LIMIT) throw Object.assign(new Error('一次导出超过 3.5GB，请分批下载'), { status: 400 });
  ...
  async function handleExportPrepare(req, res) {
    try {
      const payload = JSON.parse((await readBody(req, RPC_BODY_LIMIT)).toString('utf8'));
      const entries = await prepareExport(payload?.ids, payload?.libraryPath);
      for (const [token, plan] of exportPlans) if (plan.expires < Date.now()) exportPlans.delete(token);
      if (exportPlans.size >= 128) exportPlans.delete(exportPlans.keys().next().value);
      const token = crypto.randomBytes(24).toString('hex');
      exportPlans.set(token, { entries, expires: Date.now() + 120000 });
      return sendJSON(res, 200, { ok: true, count: entries.length, missing: 0, url: `/export?token=${token}` }, { 'Cache-Control': 'no-store' });
    } catch (error) {
      return sendJSON(res, error.status || 400, { ok: false, message: error.message, missingIds: error.missingIds });
    }
  }
  ...
  async function handleExport(req, res, url) {
    ...
    try {
      ...
      if (token) {
        const plan = exportPlans.get(token);
        if (!plan || plan.expires < Date.now()) throw Object.assign(new Error('下载准备已过期，请重新选择下载'), { status: 410 });
        entries = plan.entries;
        // Never resolve IDs again against a possibly different active library.
        for (const entry of entries) {
          const stat = await fsp.stat(entry.filePath).catch(() => null);
          if (!stat?.isFile() || stat.size !== entry.size || stat.mtimeMs !== entry.mtimeMs) {
            throw Object.assign(new Error('准备后原文件发生变化，请重新下载'), { status: 409 });
          }
        }
      } else {
        entries = await prepareExport(String(url.searchParams.get('ids') || '').split(','), url.searchParams.get('libraryPath'));
      }
    } catch (error) {
      return sendJSON(res, error.status || 400, { ok: false, message: error.message, missingIds: error.missingIds });
    }
  ...
  async function handleRequest(req, res) {
    ...
    if (urlPath === '/export/prepare' && req.method === 'POST') return handleExportPrepare(req, res);
