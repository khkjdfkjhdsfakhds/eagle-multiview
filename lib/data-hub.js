  ...
  async poll() {
    ...
    try {
      ...
      if (libraryChanged || metadataChanged) {
        try {
          library.folders = await this.client.folderTree();
        } catch {
  ...
  async mutateFolder({ id, name, baseName, force = false, origin = null, libraryPath = null }) {
    return this.enqueue(`folder:${id}`, async () => {
      ...
      if (!force && current.name !== baseName) {
        ...
        this.library = library;
        this.emit('library-changed', { library, libraryChanged: false, source: 'eagle' });
        return {
          ok: false,
          conflict: true,
          current,
      ...
      const [library, folders] = await Promise.all([this.client.libraryInfo(), this.client.folderTree()]);
      library.folders = folders;
      this.library = library;
      this.lastModificationTime = library.modificationTime;
      const updated = findFolder(folders, id);
      this.emit('library-changed', { library, libraryChanged: false, source: 'multiview', origin });
